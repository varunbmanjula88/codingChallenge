...
import PreviousPage from "./previousPageButton";
...
class CharacterDetails extends Component {
  async componentDidMount() {
    ...
    try {
      if (!this.props.selectedCharacter.id) {
        // get character details when the page is first opened directly from getCharacter api
        ...
        // character details are already exist from the previous component (HomeContainer)
      ...
      episodesList =
        episodesList instanceof Array ? episodesList : [episodesList];
  ...
  getEpisodesIds(characterDetails) {
    // returns an array of ids like [1, 6, 4, etc...]
    if (
      !checkValue(characterDetails.episode) ||
      !(characterDetails.episode instanceof Array)
    ) {
      return [];
    }

    return characterDetails.episode
      .slice(
        characterDetails.episode.length - this.props.episodesLimit,
  ...
  render() {
    return (
      <div className="character-container">
        ...
        <PreviousPage to="/" />
        {checkValue(this.props.selectedCharacter) && <CharacterInfoComponent />}
      </div>
    );
  }
}
...
export default connect(state => state, mapDispatchToProps)(CharacterDetails);
